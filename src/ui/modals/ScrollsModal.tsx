...
export function ScrollsModal({ isOpen, onClose }: ScrollsModalProps) {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Scroll Content" position="bottom">
            <div className="border border-zinc-900 bg-black p-4 text-[#e8d4b8] text-sm space-y-2 overflow-y-auto touch-pan-y h-full">
                {keptScrolls.length > 0 ? (
                    ...
                    <div className="opacity-50 text-center mt-4 pt-4 text-[#e8d4b8]">No scrolls collected</div>
                )}
            </div>
        </Modal>
    );

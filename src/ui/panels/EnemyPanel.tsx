...
function EnemyColumn({ name, hp, maxHp, atk, lvl, type, isVisible, animStatus }: EnemyColumnProps) {
    ...
    return (
        <motion.div
            className="w-full flex items-center justify-between px-2 py-1.5 sm:px-4 sm:py-3 bg-zinc-950/60 rounded-xl border border-zinc-800/80 shrink-0 shadow-lg min-h-0"
            initial={{ opacity: 1, scale: 0.95 }}
            exit={{ opacity: 0, scale: 0.9, transition: { duration: 0.2 } }}
            animate={{ opacity: 1, scale: 1 }}
        >
            {/* Left side: Icon + Type */}
            <div className="flex flex-col items-center justify-center shrink-0 w-14 sm:w-20">
                <motion.div
                    animate={animStatus}
                    variants={enemyIconVariants}
                    initial="idle"
                    className="relative flex items-center justify-center h-10 sm:h-14"
                >
                    <Icon name={name} scale={2.8} tintColor={ICON_THEME[name]} />
                </motion.div>
                {hasType && (
                    <span className="text-zinc-500 uppercase tracking-widest leading-none mt-1 sm:mt-2 text-[9px] sm:text-[10px] text-center font-bold">
                        {type}
                    </span>
                )}
            </div>
            ...
            <div className="flex flex-col flex-1 pl-3 sm:pl-4 justify-center">
                <div className="text-zinc-300 capitalize mb-1 border-b border-zinc-800/50 pb-0.5 sm:pb-1" style={{ fontSize: 'var(--text-sm)', letterSpacing: '0.05em' }}>
                    <span className="font-semibold">{name.replace('-', ' ')}</span>
                </div>

                <div className="flex items-center justify-between w-full text-[var(--text-xs)] tracking-wider mb-0.5">
                    <span className="font-semibold text-zinc-500">HP</span>
                    <motion.span animate={animStatus === 'hurt' ? 'hurt' : 'idle'} variants={hpVariants} className="font-bold text-red-400">
                        {hp} / {maxHp}
                    </motion.span>
                </div>
                ...
                <div className="flex items-center justify-between w-full text-[var(--text-xs)] tracking-wider mb-0.5">
                    ...
                    <span className="font-bold text-orange-400">{atk}</span>
                </div>
                ...
                <div className="flex items-center justify-between w-full text-[var(--text-xs)] tracking-wider">
                    ...
                    <span className="font-bold text-blue-400">{lvl}</span>
                </div>
            </div>
        </motion.div>
    );
...
export function EnemyPanel({ enemy1Anim, enemy2Anim, isBattleRunning, isPostBattleScreen, sliderResetKey, onEngage }: EnemyPanelProps) {
    ...
    return (
        <div className="flex-1 flex flex-col h-full min-h-0 min-w-0" style={{ padding: 'var(--gap)' }}>
            {/* Enemy stack */}
            <div className="flex-1 flex items-stretch justify-center min-h-0 pb-3 mt-1">
                {isPostBattleScreen ? (
                    ...
                    <div className="w-full flex flex-col justify-center gap-3 px-1 sm:px-4 max-w-sm mx-auto h-full">
